...
# Save the plot
ggsave("roles_endo.pdf", width = 8, height = 6, units = "in", device = cairo_pdf)
fig3b_endo
dev.off()
...
ggsave("roles_rhizo.pdf", width = 8, height = 6, units = "in", device = cairo_pdf)
fig3b_rhizo
dev.off()
...
##
all_varimp_rhizo <- rbind(phona_ng_rhizo$glm_output,
                    phona_sg_rhizo$glm_output,
                    phona_rst_rhizo$glm_output,
                    phona_maxi_rhizo$glm_output)
all_varimp_rhizo["Compartment"] <- "Rhizosphere"

all_varimp_endo <- rbind(phona_ng_endo$glm_output,
                          phona_sg_endo$glm_output,
                          phona_rst_endo$glm_output,
                          phona_maxi_endo$glm_output)
all_varimp_endo["Compartment"] <- "Endosphere"



all_varimp <- rbind(all_varimp_rhizo,all_varimp_endo)

write.csv(all_varimp, "all_varimp.csv")
